  ...
  for(i = 1;i <= 2*m;++i)
      ...
      if(Dos.IsTree(ee))
          ...
          if(Dos.vin[b] == Dos.vin[es] || Dos.vin[b] == Dos.vin[-es])
              ...
              h[es] = Max((double)level[Dos.vin[b]] + vpush/_beta,h[es]);
          }
          ...
          if (es==0)
              ...
              h[es] = Max(h[ee] + epush/_beta,h[es]);
          else
              h[es]=Max(h[ee] + extpush/_beta,h[es]);
          }
      }
  return h[0];
  }
...
void ComputeAngles(TopologicalGraph &G, svector<int> &level, int
  ...
  // x occupe (avec ses fils) un angle de base[x] � base[x]+angle[x]
  ...
  for (i=2; i<=2*m-n+2; i++)
      {x=liste[i];
      l=level[father[x]]+1;
      // level n'est calcul� que pour les sommets et x!=1
...
double pigaleComputeAngles(TopologicalGraph &G, svector<int> &level, int
  ...
  for (v=2;v<=n;v++)
      _beta=Max(_beta,angle[v]/(2*acos((double)level[v]/(level[v]+0.5))));
  _beta=Max(_beta,angle[1]/(2*PI));
  int x;
